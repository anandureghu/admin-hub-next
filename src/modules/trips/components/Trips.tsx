  ...
  return (
    <div className="flex flex-col h-full gap-6 overflow-hidden">
      <div className="shrink-0">
        <h1 className="page-header">{t("trips.title")}</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto flex-1">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-full sm:w-40 bg-input border-border">
              <SelectValue placeholder={t("trips.allStatus")} />
            </SelectTrigger>
            <SelectContent className="bg-card border-border">
              <SelectItem value="all">{t("trips.allStatus")}</SelectItem>
              <SelectItem value="STARTED">{t("trips.inProgress")}</SelectItem>
              <SelectItem value="ENDED">{t("trips.completed")}</SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="flex-1 min-h-0 space-y-4 overflow-y-auto pr-2 pb-4 custom-scrollbar">
        ...
        <div ref={sentinelRef} className="py-4 flex justify-center">
          ...
          {!hasNextPage && trips.length > 0 && !isLoading && (
            <p className="text-sm text-muted-foreground">{t("trips.allLoaded")}</p>
          )}
        </div>
      </div>
