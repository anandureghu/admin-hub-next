...
export const useTripsQuery = (status?: string, userIds?: string[], dateRange?: DateRange) => {
  ...
  useEffect(() => {
    queryClient.setQueryData<InfiniteData<TripPage>>(
      tripKeys.get(status, userIds, dateRange?.from ? { from: dateRange.from, to: dateRange.to } : undefined),
      (oldData) => {
  ...
  return useInfiniteQuery<TripPage>({
    queryKey: tripKeys.get(status, userIds, dateRange?.from ? { from: dateRange.from, to: dateRange.to } : undefined),
    queryFn: ({ pageParam = 0 }) =>
      tripApi.get(pageParam as number, status, userIds, dateRange?.from ? { from: dateRange.from, to: dateRange.to } : undefined),
