    ...
    return (
        <div className={cn("space-y-4 w-full", className)}>
            ...
            <div className="flex items-center gap-2">
                <Input id="vehicle-image" ref={fileInputRef} type="file" accept="image/jpeg, image/png" className="hidden" onChange={(e) => handleFileChange(e)} />
                <Button type="button" variant="outline" className="flex-1" onClick={handleClick}>
                    {preview ? "Change Image" : "Upload Image"}
                </Button>
                {preview && <Button type="button" variant="outline" className="flex-1" onClick={(e) => handleRemove(e)}>
                    Remove Image
                </Button>}
            </div>
